...
module.exports = class LinkedInRestClient {
    ...
    getAuthorizationUrl(scope, state) {
        ...
        const redirectUri = "http://localhost:5000/api/linkedInToken"; //encodeURIComponent(this.redirectUri);
        // const redirectUri = encodeURIComponent(this.redirectUri);
        return `${this.oauthUrl}/authorization?response_type=code&state=${stateString}&scope=${scopeString}&client_id=${this.clientId}&redirect_uri=${redirectUri}`;
    }
    ...
    async getAccessToken(code, state) {
        ...
        const url = `${this.oauthUrl}/accessToken?grant_type=authorization_code&code=${code}&state=${state}&redirect_uri=http://localhost:5000/api/linkedInToken&client_id=${this.clientId}&client_secret=${this.clientSecret}`;
